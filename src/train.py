...
def main():
    ...
    for epoch in range(args.epoch):
        ...
        test(net, test_dataloader, const.NUM_CLASSES, epoch)
...
def train(net, criterion_xent, criterion_cent, optimizer_model, optimizer_centloss,
    ...
    for i, sample in enumerate(trainloader):
        ...
        if (i + 1) % args.print_freq == 0:
            ...
            print("Batch {}/{}\t Loss {:.6f} \t XentLoss {:.6f} ({:.6f}) \t CenterLoss {:.6f} ({:.6f})" \
                  .format( i+1, len(trainloader), loss.item(), xent_losses.item(), cent_losses.item()))
